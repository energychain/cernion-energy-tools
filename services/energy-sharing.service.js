...
const {
  ...
  VNB_RESOLVED, VNB_AMBIGUOUS, VNB_NOT_FOUND,
  ...
  // ES Step 5 — Decision
  ES_APPROVED, ES_APPROVED_WITH_CONDITIONS,
  ES_REJECTED_STRUCTURAL, ES_REJECTED_GENERATOR_INVALID, ES_REJECTED_OTHER,
  ...
  actions: {
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  communityName:    { type: 'string', description: 'Name of the Energy Sharing community', example: 'Solargemeinschaft Rheinallee' },
                  communityId:      { type: 'string', description: 'External community ID assigned by the VNB', example: 'ES-2026-001' },
                  generators:       { type: 'array', items: { type: 'object' }, description: 'List of generator participants with mastrNummer, sharePercent, optional direktvermarkter', example: [{ mastrNummer: 'SEE904837264953', sharePercent: 100, direktvermarkter: 'Next Kraftwerke GmbH' }] },
                  consumers:        { type: 'array', items: { type: 'object' }, description: 'List of consumer participants with maloId, sharePercent, optional name', example: [{ maloId: 'DE0001234567890123456789012345678', sharePercent: 100, name: 'Whg. 1' }] },
                  maxAgeMinutes:    { type: 'integer', default: 120, description: 'Datapoint freshness threshold for Weg B (minutes)' },
                  datapointTags:    { type: 'array', items: { type: 'string' }, description: 'Tags for datapoint snapshot (Weg B fallback)', example: ['twl-netze'] },
                },
              },
              examples: {
                'Minimal valid application': {
                  value: {
                    ...
                    generators: [{ mastrNummer: 'SEE904837264953', sharePercent: 100, direktvermarkter: 'Next Kraftwerke GmbH' }],
                    consumers: [
            ...
            content: {
              'application/json': {
                example: {
                  ...
                  communityId: 'ES-2026-001',
                  decision: 'APPROVED',
                  summary: { generatorsSubmitted: 1, generatorsValid: 1, generatorsInvalid: 0, consumersSubmitted: 2, dvStatus: 'all_confirmed', totalGeneratorCapacityKW: 350, findingsCount: { info: 4, warning: 0, error: 0 }, durationMs: 5200 },
                },
              },
            },
          },
    ...
    list: {
      ...
      openapi: {
        ...
        description: 'Returns validation reports stored in PouchDB, newest first. Filter by communityId.',
        tags: ['Energy Sharing Validation'],
        parameters: [
        ...
        responses: {
          200: {
            ...
            content: {
              'application/json': {
                example: { count: 1, validations: [{ id: '...', communityName: 'Solargemeinschaft Rheinallee', communityId: 'ES-2026-001', decision: 'APPROVED', createdAt: '2026-03-30T...' }] },
              },
            },
          },
        },
    ...
    get: {
      ...
      openapi: {
        ...
        description: 'Returns the full EnergyShareValidationReport including all findings, per-generator status, and snapshot reference.',
        tags: ['Energy Sharing Validation'],
        parameters: [
  ...
  methods: {
    ...
    async tryDatapointFallback(ctx, datapointTags, maxAgeMinutes) {
      ...
      try {
        ...
        if (rows.length > 0) {
          this.logger.debug(`tryDatapointFallback: using datapoint "${dp.name}" (${rows.length} rows)`);
          return rows;
        }
      } catch (err) {
    ...
    async stepIdentity(ctx, params) {
      ...
      const identity = { mastrId: gridOperatorId || null, name: 'Unknown', bdew: gridOperatorBdew || null, bnr: null };

      try {
        const lookupParam = gridOperatorId ? { mastrId: gridOperatorId } : { bdewCode: gridOperatorBdew };
        ...
        if (!canonical.mastrId && !canonical.bdew && !canonical.name) {
          findings.push(createFinding(1, 'identity', VNB_NOT_FOUND, 'error',
            'VNB not found in register',
            'vnb_lookup_codes returned no canonical identity for the provided identifier.',
            { lookupParam }, 'Verify the grid operator identifier and resubmit.', 1));
        ...
        findings.push(createFinding(1, 'identity', code, severity,
          isAmbiguous
          ...
          { identity, candidateCount: candidates.length }, null, 1));
      } catch (err) {
        // Best-effort: proceed with provided IDs when lookup unavailable
        if (!identity.mastrId && !identity.bdew) {
          findings.push(createFinding(1, 'identity', VNB_NOT_FOUND, 'error',
            'VNB identity resolution failed', err.message, {}, 'Check MCP connectivity.', 1));
          return { identity: null, findings };
        }
        findings.push(createFinding(1, 'identity', VNB_RESOLVED, 'info',
    ...
    buildGeneratorFindings(gen, inst, wrongGridArea, idx) {
      ...
      if (wrongGridArea) {
        findings.push(createFinding(2, 'generators', GENERATOR_WRONG_GRID_AREA, 'warning',
          ...
          { mastrNummer: gen.mastrNummer }, 'Verify correct VNB for this installation.', idx));
      ...
      if (status !== STATUS_IN_BETRIEB) {
        findings.push(createFinding(2, 'generators', GENERATOR_NOT_OPERATIONAL, 'error',
          `Generator ${gen.mastrNummer} not in operational status (status: ${status})`,
          'Energy Sharing generators must be "In Betrieb" (status 35).',
          { mastrNummer: gen.mastrNummer, status }, 'Activate the installation before submitting.', idx));
        genCodes.push(GENERATOR_NOT_OPERATIONAL);
      }

      const type = this.deriveType(inst);
      if (!ELIGIBLE_TYPES.has(type)) {
        findings.push(createFinding(2, 'generators', GENERATOR_TYPE_INELIGIBLE, 'error',
          ...
          { mastrNummer: gen.mastrNummer, type }, null, idx));
        genCodes.push(GENERATOR_TYPE_INELIGIBLE);
      }

      const capacityKW = parseFloat(inst.NettoNennleistung || inst.Bruttoleistung || 0);
      if (capacityKW <= 0) {
        findings.push(createFinding(2, 'generators', GENERATOR_CAPACITY_ZERO, 'error',
          `Generator ${gen.mastrNummer} has zero or missing capacity`,
          'MaStR record shows NettoNennleistung = 0 or null.',
          { mastrNummer: gen.mastrNummer, capacityKW }, 'Correct the capacity in MaStR.', idx));
        genCodes.push(GENERATOR_CAPACITY_ZERO);
      }

      const napPresent = !!(inst.nap || inst.NapMastrNummer || inst.napMastrNummer);
      if (!napPresent) {
        findings.push(createFinding(2, 'generators', GENERATOR_NO_NAP, 'warning',
          `Generator ${gen.mastrNummer} has no NAP assignment`,
          'No Netzanschlusspunkt linked in MaStR.',
          { mastrNummer: gen.mastrNummer }, 'Submit NAP assignment via MaStR portal.', idx));
        genCodes.push(GENERATOR_NO_NAP);
      }

      const meloPresent = !!(inst.MeloMastrNummer || inst.meloMastrNummer || inst.MeloId || inst.meloId);
      if (!meloPresent) {
        findings.push(createFinding(2, 'generators', GENERATOR_NO_MELO, 'warning',
          ...
          { mastrNummer: gen.mastrNummer }, 'Register MeLo via grid operator.', idx));
        genCodes.push(GENERATOR_NO_MELO);
      }

      if (genCodes.filter((c) => [GENERATOR_NOT_OPERATIONAL, GENERATOR_TYPE_INELIGIBLE, GENERATOR_CAPACITY_ZERO].includes(c)).length === 0) {
        findings.push(createFinding(2, 'generators', GENERATOR_VALID, 'info',
          `Generator ${gen.mastrNummer} passed MaStR validation`,
          `Type: ${type}, Capacity: ${capacityKW} kW, Status: InBetrieb.`,
          { mastrNummer: gen.mastrNummer, type, capacityKW, napPresent, meloPresent }, null, idx));
    ...
    async stepGenerators(ctx, identity, params) {
      ...
      if (!allInstallations && identity?.mastrId) {
        const result = await this.callMcp('cernion_installations_local', {
      ...
      for (const gen of generators) {
        ...
        if (!inst) {
          ...
          if (!secondaryMap) {
            try {
              const r = await this.callMcp('cernion_installations_local', {
                format: 'detailed', includeStats: false, limit: 5000,
              }, token);
        ...
        if (!inst) {
          findings.push(createFinding(2, 'generators', GENERATOR_NOT_FOUND, 'error',
            `Generator ${gen.mastrNummer} not found in MaStR`,
            'No installation with this MaStR number exists in the local MaStR database.',
            { mastrNummer: gen.mastrNummer }, 'Verify the MaStR number is correct.', gIdx));
          enrichedGenerators.push({
            ...gen, status: 'invalid', mastrStatus: null, capacityKW: 0,
            type: null, gridArea: null, hasDvFlag: false, dvMastrId: null,
            napPresent: false, meloPresent: false, findings: [GENERATOR_NOT_FOUND],
          });
          gIdx++;
          continue;
        }

        const { findings: gFindings, genCodes, capacityKW, type, napPresent, meloPresent } =
          this.buildGeneratorFindings(gen, inst, wrongGridArea, gIdx);
        findings.push(...gFindings);

        const isInvalid = genCodes.some((c) =>
          [GENERATOR_NOT_OPERATIONAL, GENERATOR_TYPE_INELIGIBLE, GENERATOR_CAPACITY_ZERO].includes(c)
        );

        enrichedGenerators.push({
          ...gen,
          status: isInvalid ? 'invalid' : 'valid',
          mastrStatus: parseInt(inst.einheitBetriebsstatus ?? inst.EinheitBetriebsstatus ?? 0) === STATUS_IN_BETRIEB ? 'InBetrieb' : `status:${inst.einheitBetriebsstatus ?? inst.EinheitBetriebsstatus}`,
          capacityKW,
          type,
    ...
    buildDvFindings(gen, dvLookupResult, idx) {
      ...
      if (!gen.hasDvFlag) {
        ...
        findings.push(createFinding(3, 'directMarketer', code, severity,
          isMandatory
      ...
      if (!Array.isArray(dvEntries) || dvEntries.length === 0) {
        findings.push(createFinding(3, 'directMarketer', DV_NOT_FOUND, 'error',
          ...
          'Verify the Direktvermarkter name matches the MaStR register exactly.', idx));
        return findings;
      }
      ...
      if (gen.dvMastrId && dvEntry.mastrId && gen.dvMastrId !== dvEntry.mastrId) {
        findings.push(createFinding(3, 'directMarketer', DV_MASTR_MISMATCH, 'warning',
          `DV MaStR-ID mismatch for generator ${gen.mastrNummer}`,
          `Installation DV-ID "${gen.dvMastrId}" ≠ register result "${dvEntry.mastrId}".`,
          { mastrNummer: gen.mastrNummer, installationDvId: gen.dvMastrId, registerDvId: dvEntry.mastrId },
          'Confirm the correct Direktvermarkter with the generator operator.', idx));
        return findings;
      }
      ...
      findings.push(createFinding(3, 'directMarketer', DV_VALID, 'info',
        ...
        { mastrNummer: gen.mastrNummer, direktvermarkter: gen.direktvermarkter, dvMastrId: dvEntry.mastrId || gen.dvMastrId },
        null, idx));
      return findings;
    },
      ...
      for (const gen of enrichedGenerators) {
        ...
        if (gen.direktvermarkter) {
          if (dvCache.has(gen.direktvermarkter)) {
            ...
            try {
              dvResult = await this.callMcp('cernion_direktvermarkter_lookup', {
    ...
    stepEligibility(params, enrichedGenerators) {
      ...
      if (generators.length === 0) {
        findings.push(createFinding(4, 'eligibility', NO_GENERATORS, 'error',
          'No generators submitted', 'The generators array is empty.',
          {}, 'Add at least one generator to the application.', idx++));
      }

      if (consumers.length === 0) {
        findings.push(createFinding(4, 'eligibility', NO_CONSUMERS, 'error',
          'No consumers submitted', 'The consumers array is empty.',
          {}, 'Add at least one consumer to the application.', idx++));
      }
      ...
      if (generators.length > 0) {
        ...
        if (Math.abs(genSum - 100) > 0.1) {
          findings.push(createFinding(4, 'eligibility', SHARE_SUM_GENERATORS_INVALID, 'error',
            `Generator share sum is ${genSum.toFixed(2)}% (must be 100%)`,
            'The sharePercent values of all generators must sum to exactly 100%.',
            { sum: genSum }, 'Adjust generator sharePercent values to sum to 100%.', idx++));
        }
      }
      ...
      if (consumers.length > 0) {
        ...
        if (Math.abs(conSum - 100) > 0.1) {
          findings.push(createFinding(4, 'eligibility', SHARE_SUM_CONSUMERS_INVALID, 'error',
            `Consumer share sum is ${conSum.toFixed(2)}% (must be 100%)`,
            'The sharePercent values of all consumers must sum to exactly 100%.',
            { sum: conSum }, 'Adjust consumer sharePercent values to sum to 100%.', idx++));
        }
      }
      ...
      for (const con of consumers) {
        ...
        if (!MALO_REGEX.test(maloId)) {
          findings.push(createFinding(4, 'eligibility', CONSUMER_MALO_INVALID, 'error',
            `Consumer MaLo-ID "${maloId}" has invalid format`,
            'MaLo-IDs must match DE[0-9]{31} (33 characters: "DE" + 31 digits).',
            { maloId }, 'Correct the MaLo-ID format.', idx++));
        }
        if (maloId && maloSeen.has(maloId)) {
          findings.push(createFinding(4, 'eligibility', CONSUMER_MALO_DUPLICATE, 'error',
            `Consumer MaLo-ID "${maloId}" appears more than once`,
            'Each MaLo-ID may only appear once per Energy Sharing application.',
            { maloId }, 'Remove duplicate consumer entries.', idx++));
        }
        maloSeen.add(maloId);
      }

      // Mixed grid areas (warning)
      const gridAreas = new Set(enrichedGenerators.filter((g) => g.gridArea).map((g) => g.gridArea));
      if (gridAreas.size > 1) {
        findings.push(createFinding(4, 'eligibility', MIXED_GRID_AREAS, 'warning',
          ...
          { gridAreas: [...gridAreas] }, 'Verify all generators are in the same network area.', idx++));
      }

      // Capacity limit warnings (> 1 MW per generator)
      for (const gen of enrichedGenerators.filter((g) => (g.capacityKW || 0) > 1000)) {
        findings.push(createFinding(4, 'eligibility', GENERATOR_EXCEEDS_LIMIT, 'warning',
          ...
          'Check applicable special provisions with the regulatory authority.', idx++));
      }

      const hasErrors = findings.some((f) => f.severity === 'error');
      if (!hasErrors) {
        findings.push(createFinding(4, 'eligibility', ELIGIBILITY_CONFIRMED, 'info',
          '§ 42c EnWG eligibility confirmed',
          'All structural checks passed: share sums valid, no duplicates, all MaLo formats correct.',
          { generatorCount: generators.length, consumerCount: consumers.length }, null, idx));
      }
    ...
    stepDecision(findings) {
      ...
      const structural = errors.filter((f) =>
        [NO_GENERATORS, NO_CONSUMERS, SHARE_SUM_GENERATORS_INVALID,
          SHARE_SUM_CONSUMERS_INVALID, GENERATOR_DUPLICATE].includes(f.finding)
      );
      const genErrors = errors.filter((f) =>
        f.finding.startsWith('GENERATOR_') || f.finding.startsWith('DV_')
      );
      ...
      const severity = code === ES_APPROVED ? 'info' : code === ES_APPROVED_WITH_CONDITIONS ? 'warning' : 'error';

      return createFinding(5, 'decision', code, severity,
        ...
        { errors: errors.length, warnings: findings.filter((f) => f.severity === 'warning').length },
        isApproved ? null : 'Review and resolve listed findings before resubmitting the application.',
        1);
    },
    ...
    async stepAudit(ctx, snapshotInfo, allFindings) {
      ...
      if (snapshotInfo?.id) {
        try {
          snapshotValidation = await ctx.call('datapoint.validateSnapshot',
            { id: snapshotInfo.id }, callOpts);

          if (snapshotValidation?.drift?.length > 0) {
            findings.push(createFinding(6, 'audit', SNAPSHOT_DRIFT_DETECTED, 'warning',
      ...
      findings.push(createFinding(6, 'audit', AUDIT_TRAIL_CREATED, 'info',
    ...
    async runPipeline(ctx, params) {
      ...
      let enrichedGenerators = (params.generators || []).map((g) => ({ ...g, status: 'unknown', findings: [] }));
      ...
      await runStep(4, 'eligibility', () =>
        this.stepEligibility(params, enrichedGenerators)
      );
      ...
      const totalGeneratorCapacityKW = enrichedGenerators.reduce((s, g) => s + (g.capacityKW || 0), 0);

      return {
        communityName: params.communityName || '',
        communityId: params.communityId || '',
        gridOperator: identity || { mastrId: params.gridOperatorId || null, name: 'Unknown', bdew: params.gridOperatorBdew || null },
        decision,
        summary: {
