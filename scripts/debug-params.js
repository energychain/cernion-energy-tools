...
fs.readdirSync('../services').filter(f => f.endsWith('.service.js')).forEach(f => broker.loadService('../services/' + f));
broker.start().then(() => {
  ...
  for (const svc of services) {
    ...
    for (const [aName, action] of Object.entries(svc.actions)) {
      ...
      for (const [k, v] of Object.entries(action.params)) {
        if (typeof v === 'object' && v !== null && v.values !== undefined && !Array.isArray(v.values)) {
          const valStr = typeof v.values === 'function' ? '[Function]' : String(v.values).slice(0, 120);
