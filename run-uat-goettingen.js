...
async function runTurn(turnNumber, message) {
  ...
  try {
    ...
    const reply = resultData.reply || resultData.result?.reply || resultData.synthesisText || resultData.result?.synthesisText || resultData.result?.presentation?.markdown || JSON.stringify(resultData.result, null, 2);
    
    const presentationApplied = resultData.presentationApplied || resultData.result?.presentationApplied;
...
async function runUAT() {
  ...
  await runTurn(1, "Wir haben hier bei der Stadtwerke Göttingen AG (Netzgesellschaft) eine Netzanschlussanfrage für das neue Projekt 'Leinetal-Campus'. Es geht um 15 MW Kapazität, primär für ein Rechenzentrum und eine 3 MW Wärmepumpe zur Abwärmenutzung. Kannst du bei der Prüfung unterstützen?");
  
  await runTurn(2, "Der Anschluss soll auf der 20-kV-Mittelspannungsebene erfolgen. Unser technischer Planer sagt, das zuständige Umspannwerk operiert am N-1 Limit. Ein konventioneller Ausbau dauert 4 Jahre und kostet 3,5 Mio. Euro CAPEX. Er will den Antrag aus Kapazitätsgründen ablehnen. Geht das rechtlich nach §17 EnWG so einfach?");
  
  await runTurn(3, "Guter Punkt zur flexiblen Netzanschlussvereinbarung (fNAV). Der kaufmännische Bereich will ohnehin hohe CAPEX/Stranded Assets vermeiden und die Netzentgelte sofort sichern. Wie sieht der genaue Prozess (Rollen und Verantwortlichkeiten) aus, um diesen Konflikt zwischen Technik (will ablehnen) und Kaufmännischem Bereich (will fNAV) aufzulösen?");
  
  await runTurn(4, "Erstelle mir daraus eine VDMI-Matrix (Verantwortlich, Durchführend, Mitwirkend, Informiert) für den finalen Entscheidungsprozess und die Umsetzung dieses fNAV für den Leinetal-Campus.");
}
