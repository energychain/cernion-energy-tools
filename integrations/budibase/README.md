...
- Investment Portfolio Governance Review selector/portfolio/detail/gate/no-call rows for `ROLE_ASSET_MANAGEMENT` (#529), covering exactly three explicitly synthetic cross-division measures (Strom, Gas, Waerme; default-selected measure `smm-portfolio-gas-2026-001`), composed only from `GET /api/dashboard/investment-committee-steering-cards`, `GET /api/dashboard/investment-data-review-queue`, `GET /api/dashboard/investment-owner-deadline-budget-gate` and the reused `stadtwerk-mauer-investment-owner-deadline-budget-gate-v1` Blueprint-Pack seed via `GET /api/dashboard/stadtwerk-mauer-blueprint-pack-verify`
- Datenpunktlisten Change-Control Gate selector/alignment/test-call-readiness/kpi-cockpit/owner-deadline/verify/matrix/transfer/no-call rows for `ROLE_GRID_OPERATIONS_LEAD` (#521), covering exactly three explicitly synthetic selector cases (`existing_control_group_sufficient`, `data_point_version_or_control_group_clarification`, `test_call_evidence_or_owner_deadline_gap`), composed only from `GET /api/dashboard/controllability-data-alignment`, `GET /api/dashboard/redispatch-participation-readiness-status`, `GET /api/dashboard/redispatch-project-controlling-kpi-cockpit`, `GET /api/dashboard/owner-deadline-evidence-gate` and the reused `stadtwerk-mauer-redispatch-participation-readiness-v1` Blueprint-Pack seed via `GET /api/dashboard/stadtwerk-mauer-blueprint-pack-verify` / `GET /api/dashboard/stadtwerk-mauer-transfer-readiness`
- Infrastructure Disruption Operating Risk selector/case-impact/fallback-evidence/monitoring/freshness/de-escalation-gate/owner-deadline/interaction/no-call rows for `ROLE_GRID_OPERATIONS_LEAD` (#527), covering exactly three explicitly synthetic disruption cases (`smm-incident-datacenter-communication-001`, `smm-incident-field-terminal-fallback-002` default-selected, `smm-incident-service-provider-delay-003`), composed only from `GET /api/dashboard/communication-break-process-risk`, `GET /api/dashboard/cross-channel-vnb-signal-queue`, `GET /api/dashboard/monitoring-non-escalation`, `GET /api/dashboard/evidence-freshness-guard`, `GET /api/dashboard/automation-risk-gate` and `GET /api/dashboard/owner-deadline-evidence-gate`; no Blueprint-Pack seed or `demoProcessMatrix` is introduced for this internal panel
...
### Investment Portfolio Governance Review panel (#529)

The Investment Portfolio Governance Review panel (#529) is a generated, additive `investment_portfolio_governance_review_*` section group extending the existing investment Workbench area (it is not a duplicate cockpit). It composes only the four read models named in the accepted plan: `GET /api/dashboard/investment-committee-steering-cards`, `GET /api/dashboard/investment-data-review-queue`, `GET /api/dashboard/investment-owner-deadline-budget-gate`, and `GET /api/dashboard/stadtwerk-mauer-blueprint-pack-verify` for the reused `stadtwerk-mauer-investment-owner-deadline-budget-gate-v1` Blueprint-Pack seed (no new seed, no second `demoProcessMatrix` — the canonical verify/matrix rows are the same rows already registered for the existing Investment Owner-Frist-Budget Gate panel).

The panel answers for `ROLE_ASSET_MANAGEMENT`, in under a minute, which of exactly three explicitly synthetic cross-division measures (Strom `smm-portfolio-strom-2026-001`, Gas `smm-portfolio-gas-2026-001`, Waerme `smm-portfolio-waerme-2026-001`; IDs, CAPEX bands, owners, evidence refs, deadlines and committee windows are all explicitly synthetic and never real SAP/PSP, asset, accounting, procurement or customer records) needs human review, what evidence/budget-path/owner-deadline/committee-window fact is missing, and what safe next review gate is available. A scalar portfolio table renders one row per measure (`getInvestmentPortfolioGovernanceReview{Strom,Gas,Waerme}Row`, each independently derived from its own live `investment-data-review-queue` call); a selector row set shows the selected-measure binding (default-selected: Gas, the measure with the broadest evidence gap); detail, evidence-gap and positive-follow-up rows are composed from all three investment read models for the selected measure; and a dedicated owner/deadline/budget-path/committee-gate row set surfaces those fields explicitly. The only governance labels used are `review_ready`, `blocked_missing_evidence`, `budget_path_gap` and `needs_committee_window` — there is no numeric score, rank, weighted ranking, winner selection or automatic priority anywhere in this panel. All consequential actions (budget approval, committee execution, ERP/SAP/accounting writes, treasury transfer, workflow/HITL, MaKo/billing/settlement/tariff, device control, external connectors, Budibase table writes, Personal-Agent execution, production/public-context mutation) render only as disabled `not_called` no-call rows. The Blueprint-Pack/Cernion-Energy-Tools -> Landing-Registry -> Produktivseite sync chain is inherited unmodified from the reused verify/matrix rows, with Landing-Registry and Produktivseite explicitly pending.

### Datenpunktlisten Change-Control Gate panel (#521)

The Datenpunktlisten Change-Control Gate panel (#521) is a generated, additive `datenpunktlisten_change_control_*` section group extending the existing Redispatch E2E Workbench area (it is not a second Redispatch matrix or a new datapoint-list persistence layer). It composes only the six read models named in the accepted plan: `GET /api/dashboard/controllability-data-alignment` for data-point-list version/source snapshot, control-group/asset scope and Asset/MaStR/internal match; `GET /api/dashboard/redispatch-participation-readiness-status` for the expected impact on test-call readiness; `GET /api/dashboard/redispatch-project-controlling-kpi-cockpit` for project-controlling/KPI decision blockers and evidence gaps; `GET /api/dashboard/owner-deadline-evidence-gate` for verifying role, owner, due date and missing evidence; and `GET /api/dashboard/stadtwerk-mauer-blueprint-pack-verify` / `GET /api/dashboard/stadtwerk-mauer-transfer-readiness` for the reused `stadtwerk-mauer-redispatch-participation-readiness-v1` Blueprint-Pack seed verification, the canonical V/D/M/I matrix (`roleLegend.M = Mitwirkend`) and transfer-readiness rows. No new endpoint, service action, persistence model, import path, process command or second `demoProcessMatrix` is introduced.

The panel answers for `ROLE_GRID_OPERATIONS_LEAD`, in under a minute, whether one of exactly three explicitly synthetic selector cases (`existing_control_group_sufficient`, `data_point_version_or_control_group_clarification` — default-selected, `test_call_evidence_or_owner_deadline_gap`; case ids, data-point-list versions, control-group ids, owners, dates and evidence refs are all explicitly synthetic and marked `synthetic_tenant_seed`) really needs a data-point-list/control-group change, which evidence is missing, who verifies it, and what safe review gate follows next. The selector is presentation state only and is never persisted. Change requirement is limited to exactly three states — `not_required`, `clarification`, `evidence_gap` — derived from the existing `controllability-data-alignment` status; there is no numeric score, rank or automatic approval anywhere in this panel. A provider/Dienstleister assumption is rendered only as an evidence pointer, never as an externally verified fact. Missing evidence always maps to `clarification`/`human_review_required` with a positive follow-up, never to a bare negative conclusion. No Excel/list import, data-point-list write, Asset-MDM/MaStR mutation, test call, Redispatch enrolment/dispatch, SMGW/CLS/device control, MaKo, billing, settlement, tariff, provider call, webhook, workflow/HITL/task creation, arbitrary Budibase table write, Personal-Agent branch or direct Rundeck execution is introduced; the Blueprint-Pack/Cernion-Energy-Tools -> Landing-Registry -> Produktivseite sync chain is inherited unmodified from the reused verify/matrix rows, with Landing-Registry and Produktivseite explicitly pending.

### Infrastructure Disruption Operating Risk panel (#527)

The Infrastructure Disruption Operating Risk panel (#527) is a generated, additive `infrastructure-disruption-operating-risk-panel` read target, not a new incident-management backend, ITSM/ticketing system or Personal-Agent branch. It composes only the six read models named in the accepted plan: `GET /api/dashboard/communication-break-process-risk` for fachlicher Prozess/betroffene Entscheidung, blockierte Entscheidung, Eskalations-/Rueckzugskriterium and Owner/Vertretung; `GET /api/dashboard/cross-channel-vnb-signal-queue` for Fallback-/Evidenzreferenz, synthetischen Dienstleister-/Systemstatus and Kommunikations-/Dedupe-Status; `GET /api/dashboard/monitoring-non-escalation` for geprueftes Monitoring, Neuheit und dokumentiert abwesenden Blocker; `GET /api/dashboard/evidence-freshness-guard` for Evidenz-Aktualitaet und Delta-Status ohne automatische Eskalation; `GET /api/dashboard/automation-risk-gate` for das naechste sichere Entstoerungs-/Abschluss-Gate (Stopp-/Rollback-Kriterium); and `GET /api/dashboard/owner-deadline-evidence-gate` for Owner, Frist und positive Klaerungsschritte. No Blueprint-Pack seed, `demoProcessMatrix`, Landing-Registry item or productive Demo-Raum page is created for this internal panel — that remains a separate, later canonical Blueprint issue.

The panel answers for `ROLE_GRID_OPERATIONS_LEAD` (with contributing/informed roles surfaced per case, e.g. `ROLE_METERING`/`ROLE_IT_OPERATIONS`/`ROLE_MANAGEMENT`), for one selected synthetic disruption case out of exactly three (`smm-incident-datacenter-communication-001` — communication/process-impact gap; `smm-incident-field-terminal-fallback-002` — fallback-evidence review, default-selected; `smm-incident-service-provider-delay-003` — stale provider/completion evidence), which fachlicher Prozess is affected, what fallback/evidence proof is missing, and what safe de-escalation or closure gate follows next. The selector is renderer/query state only, is never persisted, and no arbitrary fourth case can be synthesized. Every query binds explicit scalar parameters consistent with the selected case; missing or stale evidence always maps to `clarification`/`human_review_required` with a positive follow-up, and is never rendered as an inferred real outage, provider failure, recovery or automatic closure. Enabled interaction classes are limited to exactly four: `select_synthetic_case`, `refresh_read_models`, `inspect_fallback_evidence` and `open_owner_clarification` (a display/navigation hint only, never a task or notification). Incident/ticket/task creation, pager/escalation, owner assignment, provider calls, mail/webhook, workflow/HITL, direct log access, monitoring mutation, arbitrary Budibase table writes, direct Rundeck execution, MaKo, billing, settlement, tariff, dispatch and device control render only as disabled `not_called` no-call rows.

**Visible-demo gate (open):** the repository implementation above is not visible-demo acceptance. The controlled apply/presentation/read-query smoke from the "Apply Locally" section should only run once an existing Stadtwerk-Mauer Budibase workspace/application target is independently proven and the operation is update-safe (`--preflight-existing`). As of this implementation, the Budibase application catalog remains empty/ambiguous, so an unattended first-create was not performed and this issue stays open pending that proof.
