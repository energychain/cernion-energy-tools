...
The Versandbatch Completion-Evidence Review panel (#517) is a generated, additive `versandbatch_completion_evidence_*` section group, not a new Versand-, Billing- or Workflow-Engine and not a second generic signal queue. It composes only the four read models named in the accepted plan: `GET /api/dashboard/cross-channel-vnb-signal-queue` for batch/case-group identity and channel/status reference; `GET /api/dashboard/owner-deadline-evidence-gate` for manual handover state, owner role, due date, exception/missing-evidence/blocked-decision rows and positive follow-ups; `GET /api/dashboard/evidence-freshness-guard` for return/response freshness, snapshot-delta comparison and the completion/receipt evidence pointer; and `GET /api/dashboard/automation-risk-gate` for an automation-risk summary rendered only as context. No new backend endpoint, service action, Capability Broker route, Hydration Registry rule, formatter, Personal-Agent hardcoding, scheduler, notification, connector or persistence model is introduced — this is manifest composition only.

The panel answers for `ROLE_PROCESS_OWNER`, in under a minute, whether one of exactly three explicitly synthetic selector cases (`release_ready_receipt_missing`, `manual_handover_owner_due_gap` — default-selected, `return_or_completion_evidence_stale`; batch ids, case groups, document versions, channels, owners, dates, snapshot hashes and evidence refs are all explicitly synthetic and marked `synthetic_tenant_seed`) is checkably complete by document status, release, channel/status reference, return, owner/deadline and completion evidence — or which positive human clarification is needed next. `ROLE_MARKET_OPERATIONS` contributes channel/status evidence and `ROLE_REGULATORY_AFFAIRS` is informed via the automation-risk-context row; these are transferable role families, not hardcoded people or Personal-Agent identities. The selector is presentation state only and is never persisted. Channel and shipment-status values are always rendered as caller-supplied evidence, never as a verified delivery fact. Missing or stale evidence always maps to `clarification`/`human_review_required` with a positive follow-up, never to automatic rejection, successful delivery, legal completion, billing correctness or settlement completion. No email/document sending, resend, delivery confirmation, billing, invoicing, posting, payment, settlement, MaKo, Redispatch dispatch, tariff/legal determination, mailbox/portal/ERP/CRM connector, customer contact, webhook, workflow/HITL/task creation, escalation, arbitrary Budibase table write, production data or Personal-Agent hardcoding is introduced. A visible Budibase apply/presentation smoke for this panel remains blocked pending an independently proven update-safe application/workspace target (see #572 or equivalent evidence); the repository-side manifest, tests and documentation are complete and verified without it.
