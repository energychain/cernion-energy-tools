...
- Digitalprogramm Protokoll Feedback Gate selector/decision/owner-deadline/committee/steering-acceptance/no-call rows for `ROLE_PROGRAMMSTEUERUNG` (#565), covering exactly three explicitly synthetic bounded selector items (`feedback_or_correction_open`, `owner_or_deadline_evidence_gap`, `closure_or_committee_review_ready`; default-selected item `synthetic-jf-topic-001`), composed only from `GET /api/dashboard/jour-fixe-decision-closure`, `GET /api/dashboard/gremiencoach-workbook-readiness`, `GET /api/dashboard/steering-artifact-acceptance-gate` and `GET /api/dashboard/owner-deadline-evidence-gate`
...
### Digitalprogramm Protokoll Feedback Gate panel (#565)

The Digitalprogramm Protokoll Feedback Gate panel (#565) is a generated, additive `digitalprogramm_protokoll_feedback_*` section group over #531's docs-only evidence contract (`docs/use-cases/digitalprogramm-protokoll-feedback-gate.md`). It is an internal role-workbench view, not a public Demo-Raum Macro-App candidate, so it introduces no `demoProcessMatrix`, Landing-Registry entry or Blueprint-Pack seed. It composes only the four read-only routes named in the accepted plan: `GET /api/dashboard/jour-fixe-decision-closure` (protocol item, decision status, feedback/correction, owner, evidence, closure proof, next gate), `GET /api/dashboard/owner-deadline-evidence-gate` (owner, due date, overdue/evidence state, blocked decision), `GET /api/dashboard/gremiencoach-workbook-readiness` (committee/workbook context) and `GET /api/dashboard/steering-artifact-acceptance-gate` (artefact acceptance state, escalation criterion, rollout decision).

The panel renders a bounded selector with exactly three explicitly synthetic items — `feedback_or_correction_open`, `owner_or_deadline_evidence_gap`, `closure_or_committee_review_ready` (default-selected: `synthetic-jf-topic-001`, `feedback_or_correction_open`) — for `ROLE_PROGRAMMSTEUERUNG` (V/D), while `ROLE_FACHBEREICH` / `ROLE_IT_GOVERNANCE` (M) contribute owner/deadline, committee and steering-artifact rows and `ROLE_MANAGEMENT_REVIEW` (I) appears only as an informed, disabled no-call row. Missing evidence always renders as `human_review_required` / `evidenceClass: 'clarification'` plus a positive follow-up row, never a negative factual conclusion or automatic closure. Protocol/document mutation, task creation, owner assignment, approval/rejection, mail/webhook, M365/Teams/SharePoint/calendar/transcript connectors and workflow/HITL execution render only as disabled `not_called` no-call rows; Budibase never performs an arbitrary table write.
