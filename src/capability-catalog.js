...
const CURATED_CAPABILITIES = [
  ...
  // ── Domain 1: Netzanschluss & BESS ───────────────────────────────────────
  {
    ...
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true },
      { name: 'installations', label: 'Anlagen-Liste', type: 'array', required: true }],
    risksAndNotes: ['Geo-Check basiert auf übergebenen Polygonen — VNB muss Netzgebiet als GeoJSON bereitstellen.'],
  ...
  },
  {
    ...
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['N-1-Schwellwerte sind konfigurierbar; Standardwerte: 90% kritisch, 70% Warnung.'],
  ...
  },
  {
    ...
    preferredActions: ['capex-prioritization.analyze', 'capex-prioritization.list', 'capex-prioritization.get'],
    fallbackActions: ['capex-prioritization.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Forbidden assumption field muss explizit ausgefuellt sein — keine hypothetischen CAPEX-Zahlen.'],
  ...
  },
  {
    ...
    preferredActions: ['connection-rejection-evidence.create', 'connection-rejection-evidence.list', 'connection-rejection-evidence.get'],
    fallbackActions: ['connection-rejection-evidence.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['SHA-256 auditHash ist deterministisch — Payload-Aenderungen invalidieren Hash.'],
  ...
  },
  {
    ...
    preferredActions: ['e2e-connection-check.create', 'e2e-connection-check.updateThread', 'e2e-connection-check.list'],
    fallbackActions: ['e2e-connection-check.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Alle 6 Standard-Threads muessen besetzt werden; COMPLETED erfordert alle Threads gruen.'],
  ...
  // ── Domain 2: fNAV & Flexibilitaet ────────────────────────────────────────
  {
    ...
    preferredActions: ['fnav-commercial-hedging.createContract', 'fnav-commercial-hedging.createScenario', 'fnav-commercial-hedging.listContracts'],
    fallbackActions: ['fnav-commercial-hedging.listContracts'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Compensation rates sind vertragsspezifisch — generische Defaultwerte nur als Naerung.'],
  ...
  },
  {
    ...
    preferredActions: ['flexibilitaetskosten-raster.create', 'flexibilitaetskosten-raster.list', 'flexibilitaetskosten-raster.get'],
    fallbackActions: ['flexibilitaetskosten-raster.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Signalprioritaeten 1-6 sind normativ — Abweichungen brauchen explizite Begruendung.'],
  ...
  },
  {
    ...
    preferredActions: ['reporting-governance.check', 'reporting-governance.list', 'reporting-governance.get'],
    fallbackActions: ['reporting-governance.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Maturity-Score 0-100: >=80 = GOVERNED/AUTOMATED, <40 = sofortiger Handlungsbedarf.'],
  ...
  // ── Domain 4: Systemwechsel & Assessment ─────────────────────────────────
  {
    ...
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Fotojahr 2026 hat Bonus-Scoring — Eintraege nahe 2026 werden bevorzugt klassifiziert.'],
  ...
  },
  {
    ...
    preferredActions: ['vnb-100-tage-assessment.assess', 'vnb-100-tage-assessment.list', 'vnb-100-tage-assessment.get'],
  ...
  },
  {
    ...
    preferredActions: ['altdaten-assessment.assess', 'altdaten-assessment.list', 'altdaten-assessment.get'],
  ...
  },
  {
    ...
    domain: 'asset_management',
    keywords: ['automatisierung', 'prozesskettenanalyse', 'quick win', 'manual handoff', 'automation radar'],
    preferredActions: ['automatisierungsradar.analyze', 'automatisierungsradar.list', 'automatisierungsradar.get'],
  ...
  // ── Domain 5: Gasnetz & Waermeplanung ─────────────────────────────────────
  {
    ...
    preferredActions: ['gasnetz-waermeplanung.reconcile', 'gasnetz-waermeplanung.list', 'gasnetz-waermeplanung.get'],
    ...
    risksAndNotes: ['Stranded-Risk HIGH = sofortiger TOTEX-Investitionsstopp empfohlen. H2-Kandidaten nicht abreissen.'],
  ...
  // ── Domain 6: VDMI & Regulatorik ─────────────────────────────────────────
  {
    ...
    preferredActions: ['netzkoppelvertrag-workflow.create', 'netzkoppelvertrag-workflow.updateStatus', 'netzkoppelvertrag-workflow.list'],
  ...
  },
  {
    ...
    domain: 'governance',
    keywords: ['vdmi', 'portfolio gate', 'investitionsfreigabe', 'governance', 'forbidden assumption'],
    preferredActions: ['vdmi-portfolio-gatekeeping.gate', 'vdmi-portfolio-gatekeeping.list', 'vdmi-portfolio-gatekeeping.get'],
  ...
  },
  {
    ...
    domain: 'governance',
    keywords: ['raci', 'vdmi vorlage', 'governance template', 'rollenmatrix', 'interface definition'],
    preferredActions: ['vdmi-governance-templates.instantiate', 'vdmi-governance-templates.listBaseTemplates', 'vdmi-governance-templates.listInstances'],
    ...
    risksAndNotes: ['Rollenbesetzung ueber roleAssignments — anonymisierte Basismatrizen enthalten keine Namen.'],
    routingPattern: 'governance_template_instantiate',
  },
  {
    capability: 'regulatorische_entgeltlogik',
    domain: 'regulatorik',
    keywords: ['entgeltlogik', 'netzentgelt', 'aregv', 'stromnev', '14a', 'tarifversionen', 'billing rules'],
    preferredActions: ['regulatorische-entgeltlogik.registerRuleSet', 'regulatorische-entgeltlogik.getActive', 'regulatorische-entgeltlogik.list'],
    fallbackActions: ['regulatorische-entgeltlogik.list'],
    avoid: ['query.ask'],
    requiredInputs: [{ name: 'gridOperatorId', label: 'Netzbetreiber ID', type: 'string', required: true }],
    risksAndNotes: ['Test Cases muessen 100% passen vor Produktionseinsatz — allPassed=false = Blocking.'],
...
const GLOBAL_DO_NOT_USE = [
  {
    action: 'query.ask',
    reason: 'Nur nutzen, wenn kein deterministischer oder domaenenspezifischer Pfad verfuegbar ist.',
  },
  {
