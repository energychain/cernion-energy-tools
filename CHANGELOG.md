...
### Added
- **Budibase flexible grid-connection release-file Workbench selector and sync panel** (`integrations/budibase/manifests/stadtwerk-mauer-workbench.json`, `tests/budibase-workbench.test.js`, #434, #424): The generated, read-only `flexible-grid-connection-release-file-panel` composing `GET /api/dashboard/stadtwerk-mauer-blueprint-pack-verify`, `GET /api/dashboard/stadtwerk-mauer-transfer-readiness` and `GET /api/dashboard/anschlusskapazitaet-evidence-queue` for the `stadtwerk-mauer-flexible-grid-connection-release-file-v1` seed (#424) now tags every emitted row with `renderTarget: 'budibase:stadtwerk-mauer-workbench:flexible-grid-connection-release-file-panel'`, matching the convention used by every other Workbench panel. No behavioural, endpoint or seed change.
