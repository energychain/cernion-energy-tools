...
### Added
- **FCA Process Twin QA/test coverage** (`tests/dashboard-api.test.js`, `docs/qa/fca-process-twin-qa.md`, #447): Adds focused regression coverage and maintainer QA notes for the additive `lifecycleEvidence` projection on `dashboard-api.fnavFastTrackContractGateStatus`, including non-promoting lifecycle evidence when legacy/core gate blockers remain, validation rejection for empty lifecycle references, read-only safety checks and the manual API QA checklist.
