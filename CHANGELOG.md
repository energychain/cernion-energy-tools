...
### Added
- **Wire Willi-Mako knowledge into read-only CET consumers** (`services/dashboard-api.service.js`, `services/decision-frame.service.js`, `tests/dashboard-api.test.js`, `tests/decision-frame.service.test.js`, #496): Adds an optional, advisory `makoKnowledgeContext` (Willi-Mako structural hints via `ctx.call('willi-mako.resolveStructure', ...)`) to two read-only consumers, each behind an explicit opt-in flag defaulting to `false`: `dashboard-api.marketCommunicationEvidenceChainStatus` (`includeMakoKnowledge` query param, queried with a fixed UTILMD/Marktkommunikation topic) and `decision-frame.get` (`includeMakoKnowledge` query param, queried with the frame's own `question`). Both consumers add a `maybeAttachMakoKnowledge(ctx, enabled, query)` method that is a no-op when disabled and degrades to `{ available: false, error }` — without failing the endpoint or leaking secrets — on any Willi-Mako/MCP failure. Default response shape and caching are unaffected when the flag is omitted; `willi-mako` remains the only service that knows the `cernion_willi_mako_search` MCP tool name. No workflow/EDIFACT message creation, validation or dispatch; no EDM/billing/settlement/tariff/MaKo/HITL/Budibase/SAP/device-control/external-connector mutation; no binding legal/regulatory decision.
- **Willi-Mako Marktkommunikation MCP wrapper service** (`services/willi-mako.service.js`, `tests/willi-mako.service.test.js`, #494): Adds a dedicated microservice for read-only Willi-Mako / Marktkommunikation article/context search, kept separate from the generic `knowledge-rag` service. `willi-mako.search` (`POST /api/willi-mako/search`) calls the internal MCP tool `cernion_willi_mako_search` via `CernionMCPClient.callWithNewSession` and normalizes both the documented `data.results[]` shape and a top-level `results[]` fallback into a stable `{ source, query, returned, totalScanned, warning, results[] }` response, stripping full article `content` unless `includeContent=true` is requested. `willi-mako.resolveStructure` (`POST /api/willi-mako/resolve-structure`) calls `willi-mako.search` internally and returns a conservative structure-oriented view (`sources[]`, `structuralHints[]`, `validationCandidates[]`, `noCallBoundaries[]`, `confidence`) for MaKo validation planning — it never sends, validates, or dispatches a MaKo message, and never makes a binding legal/regulatory decision. Missing `query` and `limit > 20` are rejected at the parameter-schema level; a missing `CERNION_TOKEN` or MCP failure degrades to `{ success: false, error }` without leaking secrets. `openapi-export.json`, `operation-capability-index.json` and `llm.txt` regenerated (added `'Willi-Mako Marktkommunikation': 'market-data'` to `src/llm-manifest-taxonomy.js`'s OpenAPI-tag domain map).
