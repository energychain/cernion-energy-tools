...
### Added
- **Open WebUI Sidecar bridge hardening** (`integrations/open-webui/cernion-sidecar-bridge.js`, `integrations/open-webui/docker-compose.yml`, #425): Adds a configurable OpenAI-compatible Sidecar bridge for Open WebUI with env-driven manifest/ask/plan URLs, model id and session expiry, `/health` configuration status without secret leakage, HTTP 410 fail-closed handling for expired sessions, per-conversation `turnId` / `parentTurnId` isolation, safe ask-vs-plan routing that does not misroute German domain terms such as `Zielnetzplanung`, local Docker Compose demo wiring, and focused node:test coverage for configuration, expiry, turn-state isolation and routing.
