# Referenz-Case: Messkonzept-Konflikt & Beratungspflichtverletzung (v0.58)

Dieses Dokument dokumentiert einen anonymisierten, hochgradig praxisrelevanten Multi-Turn-Referenzfall ("Der Fall Febesol") zur Fehlplanung bei der Anmeldung im Netzanschlussportal (NAP). Er dient als technischer und fachlicher Nachweis für den No-Code-Messkonzept-Engine-Ansatz von Cernion.
...
*   **Bestand vor der Dachplanung:**
    1.  Eine über 20 Jahre alte PV-Anlage (Volleinspeisung) mit eigenem Zähler (kurz vor oder nach dem Auslauf der regulären EEG-Vergütung).
    2.  Eine Bestands-Wärmepumpe (unterbrechbare Verbrauchseinrichtung, 7,5 kW) mit separatem Zähler für den vergünstigten Heizstrom-Tarif.
*   **Neue Installation (durchgeführt von einem Elektroinstallateur):**
    *   Neubau einer PV-Anlage (10 kWp) mit Batteriespeicher (10 kWh, 5 kW Ladeleistung) und einem intelligenten Messsystem (iMSys).
*   **Der Planungs- und Anmeldefehler des Installateurs:**
    *   Der Installateur demontierte den separaten Heizstromzähler und meldete im Netzanschlussportal (NAP) des VNB lediglich eine simple **„Zusammenlegung der Zähler“** (Messkonzept MK 10 - Einzähler-Eigenverbrauch) an.
    *   **Die Folgen:**
        1.  **Zerstörung des Sondertarifs:** Die Wärmepumpe wird nun über den teureren Haushaltsstromtarif abgebucht (Schaden ca. **1.400 € pro Jahr**). Es wurde versäumt, eine Kaskadenmessung (Messkonzept MK 40) aufzubauen.
...
curl -X POST http://localhost:3000/api/personal-agent/chat \
  ...
  -d '{
    "sessionId": "session-febesol-conflict-2026",
...
3. **Dokumentierter Schadenersatz-Hebel:** Mit dem JSON-Output von Cernion hat der Kunde ein hieb- und stichfestes, durch Gesetzesdaten gestütztes Gutachten, um Nachbesserungen beim Solarieur rechtlich durchzusetzen.
